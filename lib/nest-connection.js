            ...
            if (object && object.message && object.message.length > 0) {
                ...
                if (object) {
                    ...
                    translateProperty(object, 'ambient_motion', null, (ambientMotion, id) => {
                        if (checkDeviceExists(body, id) && this.legacyDeviceMap[id] == 'topaz') {
                            let thisDevice = body.topaz[id];
                            let detected = findBooleanByKeyHints(ambientMotion.data.property.value, ['occupancy', 'motion', 'presence', 'detected']);

                            if (detected === null) {
                                // Some protobuf variants expose only an "active" boolean for ambient motion.
                                detected = findBooleanByKeyHints(ambientMotion.data.property.value, ['active']);
                            }

                            if (detected === null) {
                                let enumValue = findEnumValue(ambientMotion.data.property.value, ['MOTION', 'OCCUPANCY', 'PRESENCE', 'STATE', 'STATUS']);
                                if (enumValue) {
                            ...
                            if (detected !== null) {
                                ...
                                this.verbose('[Protect Debug] ambient_motion', id, 'unable to parse occupancy from payload keys:', Object.keys(ambientMotion.data.property.value || {}));
                            }
                        }
                    });
