...
class Connection {
    ...
    protobufToNestLegacy(protobuf) {
        ...
        function unwrapKnownProtobufPayload(payload, proto) {
            let result = payload;
            let depth = 0;
            while (result !== null && result !== undefined && typeof(result) == 'object' && !Array.isArray(result) && depth < 8) {
                ...
                if (Object.prototype.hasOwnProperty.call(result, 'type_url') && Object.prototype.hasOwnProperty.call(result, 'value')) {
                    ...
                    if (proto) {
                        let trait = lookupTrait(proto, result.type_url);
                        let innerBuffer = objectToBuffer(inner);
                        if (trait && innerBuffer) {
                            try {
                                result = trait.toObject(trait.decode(innerBuffer), { enums: String, defaults: true });
                                depth++;
                                continue;
                            } catch (error) {
                                // fall through to generic unwrap
                            }
                        }
                    }
                    result = inner;
            ...
            if (object && object.message && object.message.length > 0) {
                ...
                if (object) {
                    ...
                    translateProperty(object, 'legacy_protect_device_info', null, (legacyProtectDeviceInfo, id) => {
                        if (checkDeviceExists(body, id) && this.legacyDeviceMap[id] == 'topaz') {
                            let info = unwrapKnownProtobufPayload(legacyProtectDeviceInfo.data && legacyProtectDeviceInfo.data.property && legacyProtectDeviceInfo.data.property.value, this.proto) ||
                                unwrapKnownProtobufPayload(legacyProtectDeviceInfo.data && legacyProtectDeviceInfo.data.property, this.proto) || {};
                    ...
                    translateProperty(object, 'ambient_motion', null, (ambientMotion, id) => {
                        if (checkDeviceExists(body, id) && this.legacyDeviceMap[id] == 'topaz') {
                            let thisDevice = body.topaz[id];
                            let motionPayload = unwrapKnownProtobufPayload(ambientMotion.data && ambientMotion.data.property && ambientMotion.data.property.value, this.proto);
                            if (!motionPayload || (typeof(motionPayload) == 'object' && Object.keys(motionPayload).length === 0)) {
                                motionPayload = unwrapKnownProtobufPayload(ambientMotion.data && ambientMotion.data.property, this.proto);
                            }
                            ...
                            if (detected !== null) {
                                ...
                                this.verbose('[Protect Debug] ambient_motion', id, 'unable to parse occupancy from payload keys:', Object.keys(motionPayload || {}), 'raw property keys:', Object.keys(rawProperty || {}), 'raw value type:', rawValue && rawValue.constructor && rawValue.constructor.name, 'raw value length:', rawValue && rawValue.length);
                            }
                        }
                    });
