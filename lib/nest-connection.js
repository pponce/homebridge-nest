...
class Connection {
    ...
    protobufToNestLegacy(protobuf) {
        ...
        function unwrapKnownProtobufPayload(payload) {
            ...
            if (Object.prototype.hasOwnProperty.call(payload, 'value') && Object.keys(payload).length <= 3) {
                if (payload.type_url || typeof(payload.value) == 'object') {
            ...
            if (object && object.message && object.message.length > 0) {
                ...
                if (object) {
                    ...
                    translateProperty(object, 'legacy_protect_device_info', null, (legacyProtectDeviceInfo, id) => {
                        if (checkDeviceExists(body, id) && this.legacyDeviceMap[id] == 'topaz') {
                            let info = unwrapKnownProtobufPayload(legacyProtectDeviceInfo.data && legacyProtectDeviceInfo.data.property && legacyProtectDeviceInfo.data.property.value) ||
                                unwrapKnownProtobufPayload(legacyProtectDeviceInfo.data && legacyProtectDeviceInfo.data.property) || {};
                            ...
                            if (autoAway !== null) {
                                ...
                                this.verbose('[Protect Debug] legacy_protect_device_info', id, 'unable to parse autoAway from keys:', Object.keys(info || {}));
                            }
                    ...
                    translateProperty(object, 'ambient_motion', null, (ambientMotion, id) => {
                        if (checkDeviceExists(body, id) && this.legacyDeviceMap[id] == 'topaz') {
                            let thisDevice = body.topaz[id];
                            let motionPayload = unwrapKnownProtobufPayload(ambientMotion.data && ambientMotion.data.property && ambientMotion.data.property.value);
                            if (!motionPayload || (typeof(motionPayload) == 'object' && Object.keys(motionPayload).length === 0)) {
                                motionPayload = unwrapKnownProtobufPayload(ambientMotion.data && ambientMotion.data.property);
                            }
